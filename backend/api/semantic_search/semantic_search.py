...
def train_search_model():
    ...
    df = pd.read_csv(PORTFOLIO_COMPANIES_FILE_PATH)
    # Remove unnecessary columns
    df.drop('TAGS', axis=1, inplace=True)
    df.drop('HQ/Main Office', axis=1, inplace=True)
    df.drop('VERTEX ENTITY', axis=1, inplace=True)
    df.drop('FINANCE STAGE', axis=1, inplace=True)
    df.drop('HOLDING STATUS', axis=1, inplace=True)
...
def main():
    query = "AI and machine learning company in the healthcare sector."
